...
# Focus on original (non-perturbed) dataset

# observed outcome 
y_obs <- survey_df$kept_name  #data dependent
...
# NOTE: respondents with y=0 tend to get larger weights
weights_df %>% 
  ggplot() +
...
weights_df %>% 
  ggplot() +
  geom_violin(aes(x = y_obs_factor, y = w, fill = y_obs_factor)) +
  xlab('y') +
  ggtitle('Weights distribution by outcome') +
  theme_bw() +
...
# NOTE: all the outliers belong to the lowest educational bin which is highly 
# underrepresented in the survey sample in comparison to the population
...
# NOTE: larger positive weights are associated to older respondents, to 
# respondents with no college degree, and to respondents that got married in the 
# first decade considered, which are all categories that are underrepresented in 
# the survey sample in comparison to the population
