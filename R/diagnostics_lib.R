...
#' @param y_new A vector of new responses, the same dimension as survey_df$y
#'
#' @export
GetLogitImportanceWeights <- function(logit_post, survey_df, y_new) {
  ...
  stopifnot(ncol(linpred) == length(survey_df$y))

  # Transpose to take advantage of column broadcasting
  ll_diff <- (t(linpred) * (y_new - survey_df$y)) %>% t() %>%
...
EvalParametricBootstrap <- function(mrpaw_list, post, survey_df) {
    ...
    boot_bias <- sum(mrpaw_list$w * (yhat - survey_df$y))
